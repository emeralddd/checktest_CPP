...
void gcc()
{
    ...
    file.close();
    if (config[0] == '!')
        compileCommandPrefix = "g++.exe";
    else
        compileCommandPrefix = quote + config + quote;
}
void testGCC()
{
...
void getCurrentPath()
{
    ...
    while (currentPath[currentPath.size() - 1] == ' ')
        currentPath.erase(currentPath.size() - 1, 1);
}
...
int main()
{
    ...
    string TASKNAME = "ahaha";

    gcc();
    testGCC();
    getCurrentPath();
    getOutputPath(TASKNAME);
